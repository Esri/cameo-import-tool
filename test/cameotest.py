...
sampleCAMEO_suite = {
    "source": sampleCAMEOzip,
    "GDBName": "SampleCAMEO",
    "GDBPath" : os.path.join(testDir, "SampleCAMEO.gdb"),
    "RelationshipClasses" : [
        'Facilities__ATTACHREL',
        'Contacts__ATTACHREL',
        'Facilities_FacilityIDs',
...
combinedCAMEO_suite = {
    "source": [sampleCAMEOzip,mockCAMEOzip],
    "GDBName": "CombinedCAMEO",
    "GDBPath" : os.path.join(testDir, "CombinedCAMEO.gdb"),
    "RelationshipClasses" : [
        'Facilities__ATTACHREL',
        'SpecialLocations__ATTACHREL',
        'Incidents__ATTACHREL',
...
def functional_tests(testSuite):
    ...
    for fcName, fcDict in testSuite['FeatureClasses'].items():
        ...
        print("Feature Class {}: \nExpected Count: {} | Actual Count: {} | {}".format(fcName, expected, actual, test5))
        
        test6 = sorted([field.name for field in arcpy.ListFields(fcName)]) == sorted(fcDict['Fields'])
        print("Expected Fields Created: {}".format(test6))

        for test in [test5, test6]:
            if test == False:
                errorCount += 1
    ...
    for tableName, tableDict in testSuite['Tables'].items():
        ...
        print("Table {}: \nExpected Count: {} | Actual Count: {} | {}".format(tableName, expected, actual, test8))
        
        test9 = sorted([field.name for field in arcpy.ListFields(tableName)]) == sorted(tableDict['Fields'])
        print("Expected Fields Created: {}".format(test9))
